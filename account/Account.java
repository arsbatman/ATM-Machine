...
public class Account {
    ...
    public static ArrayList<Account> createAccountArrayList (Customer customer) {
        ...
        try {
            stmt = DBconnection.prepareStatement("select count(account_number) from account where customer_id = ?");
            ...
            stmt = DBconnection.prepareStatement("select account_number, customer_id, account_type, account_balance from account where customer_id = ?");
            stmt.setInt(1, customer.getCustomerID());
            resultSet = stmt.executeQuery();
            ...
            while (i <= numberOfAccounts-1) {
                while (resultSet.next()) {
                    accountArray.get(i).setAccountNumber(resultSet.getInt(1));
    ...
    public boolean checkForOpenAccount(Customer customer, ArrayList<Account> accountArray, Account account) {
        ...
        try {
            ...
            //if 1 or more accounts open, display each account and its properties in console with formatting
                ...
                stmt = DBconnection.prepareStatement("select account_number, account_balance, account_type from account where customer_id = ?");
                stmt.setInt(1, customer.getCustomerID());
                resultSet = stmt.executeQuery();
    ...
    public void checkAccountNumber(Customer customer, ArrayList<Account> accountArray, Account account) {
        ...
        while (isValidAccountNumber == false) {
            ...
            choose= MScanner.getNumber();
            for (int i = 0; i < accountArray.size(); i++) {
                if (accountArray.get(i).getAccountNumber()==(choose)) {
